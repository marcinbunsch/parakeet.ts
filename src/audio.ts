...
export function loadAudioRaw(filename: string, samplingRate: number): Float32Array {
...
function hammingWindow(size: number): Float32Array {
  const w = new Float32Array(size);
  for (let i = 0; i < size; i++) w[i] = 0.54 - 0.46 * Math.cos((2 * Math.PI * i) / size);
  return w;
}
...
function bartlettWindow(size: number): Float32Array {
  const w = new Float32Array(size);
  for (let i = 0; i < size; i++) w[i] = 1.0 - Math.abs((2 * i - size) / size);
  return w;
}

export function getWindow(name: string, size: number): Float32Array {
...
function rfft(input: Float32Array, nFft: number, re: Float32Array, im: Float32Array, out: Float32Array): void {
  const N = nFft;
  re.set(input.subarray(0, Math.min(input.length, N)));
  if (input.length < N) re.fill(0, input.length);
  im.fill(0);
  ...
  const numBins = (N >> 1) + 1;
  for (let i = 0; i < numBins; i++) { out[i * 2] = re[i]; out[i * 2 + 1] = im[i]; }
}
...
function computeStft(
  ...
  window: Float32Array,
  magPower: number,
): { magnitude: Float32Array; numFrames: number; numBins: number } {
  const padding = Math.floor(nFft / 2);
  const padded = new Float32Array(signal.length + 2 * padding);
  for (let i = 0; i < padding; i++) padded[padding - 1 - i] = signal[i + 1] ?? 0;
  ...
  for (let t = 0; t < numFrames; t++) {
    ...
    for (let i = 0; i < nFft; i++) frame[i] = (padded[start + i] ?? 0) * win[i];
 ...
 */
export function computeMelFilterbanks(
  ...
  for (let i = 0; i < nMels + 2; i++) melPoints[i] = melMin + (i / (nMels + 1)) * (melMax - melMin);
  ...
  for (let m = 0; m < nMels; m++) {
    ...
    for (let k = fLeft; k < fCenter; k++) {
      if (k >= 0 && k < numBins) fb[m * numBins + k] = (k - fLeft) / Math.max(1, fCenter - fLeft);
    }
    for (let k = fCenter; k <= fRight; k++) {
      if (k >= 0 && k < numBins) fb[m * numBins + k] = (fRight - k) / Math.max(1, fRight - fCenter);
    }
  }
  for (let m = 0; m < nMels; m++) {
    const norm = 2.0 / Math.max(1e-8, hzPoints[m + 2] - hzPoints[m]);
    for (let k = 0; k < numBins; k++) fb[m * numBins + k] *= norm;
  }
  return fb;
}
 ...
 */
export function computeMelFilterbanksInterpolated(
  sr: number, nFft: number, nMels: number, fMin: number, fMax: number,
): Float32Array {
  const numBins = Math.floor(nFft / 2) + 1;
  const hzToMel = (hz: number) => 2595 * Math.log10(1 + hz / 700);
  ...
  for (let k = 0; k < numBins; k++) fftFreqs[k] = (k * sr) / nFft;
...
export function getLogMel(signalIn: Float32Array, args: PreprocessArgs): LogMel {
  let signal = signalIn;
  ...
  if (args.preemph !== null) {
    ...
    p[0] = signal[0];
    for (let i = 1; i < signal.length; i++) p[i] = signal[i] - args.preemph * signal[i - 1];
    signal = p;
  }

  const win = getWindow(args.window, args.winLength);
  const { magnitude, numFrames, numBins } =
    computeStft(signal, args.nFft, args.hopLength, args.winLength, win, args.magPower);
  ...
  for (let m = 0; m < nMels; m++) {
    ...
    for (let t = 0; t < numFrames; t++) {
      ...
      for (let k = 0; k < numBins; k++) acc += fb[fbRow + k] * magnitude[magRow + k];
  ...
  if (args.normalize === 'per_feature') {
    for (let m = 0; m < nMels; m++) {
      ...
      let mean = 0;
      for (let t = 0; t < numFrames; t++) mean += out[base + t];
      mean /= numFrames;
      let varr = 0;
      for (let t = 0; t < numFrames; t++) { const d = out[base + t] - mean; varr += d * d; }
      const inv = 1 / (Math.sqrt(varr / numFrames) + 1e-5);
      for (let t = 0; t < numFrames; t++) out[base + t] = (out[base + t] - mean) * inv;
    }
  } else {
    let mean = 0;
    ...
    mean /= out.length;
    let varr = 0;
    for (let i = 0; i < out.length; i++) { const d = out[i] - mean; varr += d * d; }
