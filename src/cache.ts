...
export class ConformerCache {
  ...
  updateAndFetchConv(x: MxArray, padding: number): MxArray {
    ...
    if (this.conv === null) {
      ...
      const tail = this.conv.slice(
        BigInt64Array.from([0n, BigInt(convLen - keep), 0n]),
        this.conv.shape(),
      );
...
export class RotatingConformerCache extends ConformerCache {
  ...
  updateAndFetchKV(k: MxArray, v: MxArray): [MxArray, MxArray] {
    ...
    if (currentLen > this.keepSize) {
      ...
      const trimmedK = cachedK.slice(
        BigInt64Array.from([0n, 0n, BigInt(start), 0n]),
        cachedK.shape(),
      );
      const trimmedV = cachedV.slice(
        BigInt64Array.from([0n, 0n, BigInt(start), 0n]),
        cachedV.shape(),
      );
