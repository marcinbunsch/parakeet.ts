...
export class ConvASRDecoder extends Module {
  ...
  constructor(args: ConvASRDecoderArgs) {
    super();
    const numClasses =
      (args.numClasses <= 0 ? args.vocabulary.length : args.numClasses) + 1;
...
export function decodeCTCGreedy(
  features: MxArray,   // [batch, seq, encoderDim]
  ...
  for (let b = 0; b < B; b++) {
    ...
    for (let t = 0; t < length; t++) {
      ...
      if (tokenIdx === vocabSize) continue; // skip blank
      if (tokenIdx === prevToken) continue;  // skip repeat
