...
export function makePreprocessArgs(args: {
  ...
  const filterbanks = computeMelFilterbanks(
    args.sampleRate,
    args.nFft,
    args.features,
    0,
...
export function loadAudio(filename: string, samplingRate: number): MxArray {
...
function getWindow(name: string, size: number): Float32Array {
...
function rfft(input: Float32Array, nFft: number): Float32Array {
...
function computeMelFilterbanks(
  sr: number,
  nFft: number,
  nMels: number,
  fMin: number,
 ...
 */
export function getLogMel(audio: MxArray, args: PreprocessArgs): MxArray {
  // Bring to CPU
  ...
  const magArray = MxArray.fromFloat32(
    magnitude,
    BigInt64Array.from([BigInt(numFrames), BigInt(numBins)]),
  );
