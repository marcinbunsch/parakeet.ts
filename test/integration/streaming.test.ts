/**
 * Streaming (sliding-window) — runs against the one backend selected by
 * PARAKEET_BACKEND. The same StreamingParakeet drives MLX and ONNX, so these
 * properties must hold either way. Ground truth is the backend's own one-shot
 * `transcribePcm` on the same audio, not a hand-written fixture.
 */
import { describe, it, expect, beforeAll } from "vitest"
import type { ParakeetModel } from "../../src/index.js"
import {
  backend, normalize, wordErrorRate, concatWithGaps, loadPcm, runStream,
} from "./support.js"
