...
by comparing recorded transcripts (`pnpm test:mac` and `pnpm test:linux` assert
the same expected text against each backend), not by running both stacks on one
machine.
