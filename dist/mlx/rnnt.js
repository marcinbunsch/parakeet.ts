...
import { Module, Linear, Embedding, LSTM, relu, sigmoid, softmax } from './nn.js';
...
export function decodeTDTGreedy(features, // [batch, seqLen, encoderDim]
lengths, // [batch]
decoder, joint, vocabulary, durations, maxSymbols, states, timeRatio) {
    ...
    for (let b = 0; b < B; b++) {
        ...
        while (step < length) {
            ...
            if (!isBlank) {
                ...
                hypothesis.push(makeAlignedToken(predToken, decode([predToken], vocabulary), step * timeRatio, durations[decision] * timeRatio, confidence));
                lastToken = predToken;
...
/** Greedy RNNT decoder */
export function decodeRNNTGreedy(features, lengths, decoder, joint, vocabulary, maxSymbols, states, timeRatio) {
    const fShape = features.shape();
    const B = Number(fShape[0]);
    const results = [];
    const newStates = [];
    for (let b = 0; b < B; b++) {
        ...
        while (step < length) {
            ...
            if (!isBlank) {
                ...
                hypothesis.push(makeAlignedToken(predToken, decode([predToken], vocabulary), step * timeRatio, timeRatio, confidence));
                lastToken = predToken;
                hiddenState = [newH, newC];
                newSymbols += 1;
