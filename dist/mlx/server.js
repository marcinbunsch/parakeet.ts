 ...
 */
export function createParakeetRoutes(options) {
    ...
    app.post('/transcribe', async (c) => {
        ...
        if (contentType.trim().toLowerCase() !== REQUIRED_CONTENT_TYPE) {
            return c.json({ error: 'unsupported_media_type', message: `Content-Type must be "${REQUIRED_CONTENT_TYPE}"` }, 415);
        }
        const streamingSession = model.transcribeStream();
        let totalBytes = 0;
        let timedOut = false;
        let tooLarge = false;
        async function* bodyToPcm() {
        ...
        try {
            ...
            if (tooLarge) {
                return c.json({ error: 'payload_too_large', message: `Audio exceeds maximum of ${maxDurationSeconds} seconds` }, 413);
            }
            if (timedOut) {
                return c.json({ error: 'idle_timeout', message: `No audio received for ${idleTimeoutMs} ms` }, 408);
            }
            ...
            return c.json({ error: 'internal_error', message: 'Unexpected error during transcription' }, 500);
        }
    });
    return app;
}
